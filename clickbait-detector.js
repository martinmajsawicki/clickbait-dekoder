...
const PATTERNS = [
  {
    ...
    rules: [
      ...
      { re: /dowiedz się/i, exclude: /Sponsorowane|Promoted|Sponsored|Reklama/i, snark: '"{0}" — dowiesz się, że nie warto było się dowiadywać.' },
      ...
      { re: /ma\s+(kłopoty|problem)[.\!\s]/i, snark: '"{0}" — ale jaki? Nie powiedzą, bo wtedy nie klikniesz. Zwykle niewielki.' },
      { re: /pokazał[aoy]?\s+postanowienie/i, snark: '"{0}" — opublikował dokument. Gdyby był przełomowy, napisaliby co w nim jest.' },
      { re: /wskazuj[ąeę]\s+(przyczyn|powód|powody|źródł[oa])/i, snark: '"{0}" — wskazują przyczynę, ale jej nie podają. Gdyby była ciekawa, stałaby w tytule.' },
      { re: /padł[yo]\s+(dwa|trzy|cztery|kilka)?\s*nazwisk/i, snark: '"{0}" — padły nazwiska, ale jakie? Gdyby były ważne, stałyby w tytule. Technika: obietnica konkretu bez konkretu.' },
      { re: /zaginio\w+\s+.{0,20}odnalezio\w+\s+.{0,10}mart/i, snark: '"{0}" — nie mówią kto, gdzie, kiedy. Trzeba kliknąć, żeby się dowiedzieć, że to nie twoje miasto.' },
      { re: /każdy\s+polak\s+(zna|wie|pamięta|kojarzy|słyszał)/i, snark: '"{0}" — nie każdy. I nie daj się nakłonić do klikania, żeby sprawdzić czy Ty też znasz.' },
      { re: /nie\s+każdy\s+polak\s+(zna|wie|pamięta)/i, snark: '"{0}" — chcą, żebyś sprawdził, czy Ty wiesz. I potem jeszcze raz. I kolejny. I tak przez godzinę.' },
      { re: /^mówi\s+się,?\s+że/i, snark: '"{0}" — "mówi się" = źródło niepewne i niepoważne. Gdyby było poważne, podaliby jakie.' },
      { re: /niektórzy\s+do\s+dziś/i, snark: '"{0}" — którzy "niektórzy"? To mogą być niewidzialni przyjaciele autora tekstu.' },
      { re: /\.\s+To[…\.]{1,3}\s*["„"']?\s*$/i, snark: '"{0}" — cytat urwany w najbardziej interesującym miejscu. Celowo. Musisz kliknąć, żeby dowiedzieć się reszty.' },
      { re: /te\s+(polskie|nowe|najnowsze|stare|kultowe)\s+(filmy|książki|seriale|produkty|miejsca|miasta)/i, snark: '"{0}" — "te" zamiast tytułów. Gdyby były warte uwagi, podaliby nazwy.' },
      { re: /wydał[aoy]?\s+komunikat\.?\s*$/i, snark: '"{0}" — wydała komunikat, ale JAKI? Gdyby był ważny, jego treść byłaby w tytule.' },
      { re: /(niemcy|francuzi|włosi|japończycy|amerykanie|brytyjczycy|hiszpanie)\s+zakochali\s+się\s+w\s+(tym|tej)/i, snark: '"{0}" — ilu dokładnie? 53? 211? 900? I w CZYM się zakochali? Nie powiedzą, bo to pewnie normalne miejsce.' },
      { re: /wojna\s+we\s+władzach/i, snark: '"{0}" — czyli ktoś z kimś się nie zgadza. Dalej ciekawe?' },
      { re: /(japończycy|koreańczycy|włosi|francuzi|hiszpanie|grecy|skandynawowie|szwedzi|duńczycy|finowie)\s+to\s+(jedzą|piją|robią|stosują|używają|biorą)/i, snark: '"{0}" — CO jedzą/piją/robią? Nie powiedzą, bo wtedy Google wystarczy zamiast kliknięcia.' },
      { re: /polacy\s+nie\s+(znają|wiedzą|doceniają|używają)/i, snark: '"{0}" — Polacy nie znają, bo nie ma czego znać. Gdyby było warte uwagi, napisaliby co to jest.' },
      { re: /^(katastrofa|wypadek|tragedia|dramat)\s+(na\s+)?(lotnisku|drodze|autostradzie|torach|moście)\b/i, exclude: /\b(w\s+\w+|pod\s+\w+|koło\s+\w+|niedaleko\s+\w+)\b/i, snark: '"{0}" — ale na KTÓRYM? Nie podają, bo może się okazać, że to nie twój kraj.' },
      { re: /\b(dw[aoóje]\w*|trz[yoe]\w*|czwor\w+|pięć\w*|sześ\w+|siedm\w+|ośm\w+|dziewięć\w*|dziesię\w+|\d+)\s+\w+,\s+.{2,30}\b(dw[aoóje]\w*|trz[yoe]\w*|czwor\w+|pięć\w*|sześ\w+|siedm\w+|ośm\w+|dziewięć\w*|dziesię\w+|\d+)\s+\w+.{0,30}\bi\s+(on|ona|ono|oni|one|to)\b/i, snark: '"{0}" — technika filmowej zapowiedzi: same liczby, zero kontekstu. Kto? Co? Po co? Musisz kliknąć.' },
      ...
      { re: /^smutn[eayo]\s+doniesieni/i, snark: '"{0}" — nie chcą podać przyczyn, bo przyczyny są banalne.' },
      { re: /^plotkowano/i, snark: '"{0}" — nie wiemy kto mówił, co i czy prawdziwe.' },
      { re: /^odnios/i, snark: '"{0}" — jakby to była ważna osoba, podaliby kto.' },
      { re: /^był[aoy]?\s+gwiazd/i, snark: '"{0}" — chyba niezbyt wielką, bo nazwiska nie podali.' },
      { re: /wzbudza\s+kontrowersj/i, snark: '"{0}" — jakie kontrowersje, u kogo? Nie powiedzą, bo wtedy nie klikniesz.' },
      { re: /bij[ąa]\s+na\s+alarm/i, snark: '"{0}" — eksperci z definicji tego nie robią. Najwyżej mówią, jakie mogą być konsekwencje.' },
      { re: /słowa\s+niosą\s+się/i, snark: '"{0}" — jakie słowa? Czyje? Nie powiedzą.' },
      { re: /nie\s+zostawił[aoy]?\s+suchej\s+nitki/i, snark: '"{0}" — czyli skrytykował. Ale "skrytykował" to nie nagłówek.' },
      { re: /przemówił[aoy]?\s+po/i, snark: '"{0}" — czyli powiedział coś. Ale CO — tego tytuł nie zdradzi.' },
      { re: /ogłasza\s+ws\./i, snark: '"{0}" — ale nie mówi CO ogłasza. Bo gdyby napisali, nie kliknąłbyś.' },
      { re: /finał\s+(sprawy|historii|afery)/i, snark: '"{0}" — jakiej sprawy? Nie powiedzą. Zaskoczył? Pewnie nie.' },
      { re: /^to\s+wydarzył[aoy]\s+się/i, snark: '"{0}" — ale CO? Tytuł buduje napięcie zamiast informować.' },
      { re: /są\s+wśród\s+nas/i, snark: '"{0}" — oczywistość podana jako rewelacja.' },
      { re: /tego\s+nie\s+wiedz/i, snark: '"{0}" — i nie dowiedzą się z nagłówka. Trzeba klikać.' },
      { re: /rząd\s+szykuje\s+zmiany/i, snark: '"{0}" — żyjemy tu od lat. Zaskoczyłoby, gdyby nic się nie zmieniało.' },
      { re: /rzeczywistość\s+może\s+zaskoczyć/i, snark: '"{0}" — zbitka bez konkretów. Próbują zaskoczyć brakiem informacji.' },
      { re: /ujawnia\s+ogrom/i, snark: '"{0}" — mogli napisać kto i co, ale wybrali ogólniki.' },
      { re: /zdjęcia\s+nie\s+kłam/i, snark: '"{0}" — ale musisz kliknąć, żeby je zobaczyć.' },
      { re: /mówi\s+o\s+(ważnym|istotnym|kluczowym)\s+szczegół/i, snark: '"{0}" — podajemy ten szczegół w tytule? Jeśli nie, to pewnie nie taki ważny.' },
      { re: /^znan[yae]\s+z\s+/i, snark: '"{0}" — gdyby osoba była naprawdę znana, podaliby nazwisko.' },
      { re: /tyle\s+(seniorzy|emeryci|polacy|pracownicy|nauczyciele|rolnicy|rodzice|podatnicy|kierowcy)\s+(dostan|zapłac|otrzymaj|strac|utrac|zarobi)/i, exclude: /\d+\s*(zł|tys|mln|mld|%|złotych|procent)/, snark: '"{0}" — tyle, ale ile? Nie powiedzą, bo kwota pewnie nie robi wrażenia.' },
      { re: /spowied[źż]\s+/i, snark: '"{0}" — "spowiedź" brzmi intymnie. Treść pewnie jest normalnym wywiadem.' },
      { re: /aż\s+huczy/i, snark: '"{0}" — huczy, czyli kilka osób skomentowało. Ale "kilka osób" to nie nagłówek.' },
      { re: /klamka\s+zapadła/i, snark: '"{0}" — klamka zapadła, ale na co? Tytuł nie powie.' },
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /najlepsz[yae]/i, exclude: /nagrod[aąęy]|laureat|finalist|nominowan|konkurs|festiwal|ranking\s+\w+\s+\d{4}/i, snark: '"{0}" — w sporcie pewnie prawda. Ale tytuł nie mówi w czym i o ile — bo wtedy nie klikniesz.' },
      ...
      { re: /lawin[aąęy]\s+(komentarzy|reakcji|krytyki|hejtu)/i, snark: '"{0}" — lawina = kilka osób skomentowało. Ale "kilka komentarzy" to nie nagłówek.' },
      { re: /furor[ęą]/i, snark: '"{0}" — furorę = zwróciła uwagę. Ale "zwróciła uwagę" nie generuje kliknięć.' },
      { re: /wmurowało/i, snark: '"{0}" — wmurowało, czyli zaskoczyło. Na chwilę. Potem scrollowali dalej.' },
      ...
      { re: /stane?ł[aoy]?\s+w\s+ogniu/i, exclude: /dosłownie|pożar\w*\s+(straw|zniszcz)|ogień\s+(straw|objął)/i, snark: '"{0}" — "stanęły w ogniu" = zapaliły się lub zostały podpalone. Ale "zapaliły się" nie budzi wyobraźni tak jak wielki pożar.' },
      { re: /upokorzenie\s+(dla|całego)\s+(zachod|polsk|europ|ameryk|nato)/i, snark: '"{0}" — "upokorzenie" sugeruje celowe poniżenie. W rzeczywistości pewnie ktoś coś powiedział lub zrobił, ale bez dramatyzmu nie ma kliknięć.' },
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /napięcie\s+rosło/i, snark: '"{0}" — i trzeba je podtrzymać niepewnością w nagłówku.' },
      { re: /tak\s+się\s+(początkowo\s+)?wydawało/i, snark: '"{0}" — czyli tytuł też miał treść, i tak się początkowo wydawało.' },
      { re: /tragiczn[yae]\s+finał/i, snark: '"{0}" — ale czego? Nie powiedzą, żebyś kliknął.' },
      { re: /alarm\s+w\s+/i, snark: '"{0}" — w całym kraju? Raczej w jednym budynku. Ale "alarm w budynku" to nie nagłówek.' },
      { re: /^nocn[aąeyo]\s+schadzk/i, snark: '"{0}" — kto, kiedy, gdzie? Nie powiedzą, bo wtedy nie klikniesz.' },
      { re: /są\s+(zdjęcia|fotki|nagrania)/i, snark: '"{0}" — są, ale nie pokażą. Musisz kliknąć.' },
      { re: /chwali\s+się/i, snark: '"{0}" — czym? Nie powiedzą, bo wtedy nie klikniesz.' },
      { re: /^plotkowano/i, snark: '"{0}" — nie wiemy kto mówił, co i czy prawdziwe.' },
      { re: /bezlitoś\w*.{1,40}powołani/i, snark: '"{0}" — dramatyzacja normalnej decyzji sportowej. Trener wybrał skład — to jego praca.' },
      { re: /ukryt[aey]\s+rodzin/i, snark: '"{0}" — "ukryta rodzina" — ukryta przed kim? Technika: serialowa narracja zamiast informacji.' },
      { re: /nie\s+m[oó]g[łl][aoy]?\s+uwierzyć/i, snark: '"{0}" — nie mógł uwierzyć w CO? Tytuł ukrywa, bo samo w sobie nie robi wrażenia.' },
      { re: /się\s+wygadał[aoy]?\.?\s*$/i, snark: '"{0}" — "wygadała się" = powiedziała coś normalnego, ale redakcja dodała dramaturgii. W realu pewnie odpowiedziała na pytanie.' },
      { re: /(protestują|krytykują|żądają).{0,30}(odpowiada|reaguje|komentuje)\.\s/i, snark: '"{0}" — odpowiada, ale CO? Postawili kropkę zamiast wyjaśnić stanowisko. Celowo.' },
      { re: /chcę\s+jasno\s+powiedzieć/i, snark: '"{0}" — chce jasno powiedzieć, ale cytat urwany. Jasność jest za paywallem kliknięcia.' },
    ],
  },
  {
    id: 'collective',
    ...
    rules: [
      ...
      { re: /odwrócili\s+się\s+od/i, snark: '"{0}" — wszyscy? Pewnie kilku. Ale "kilku" to nie nagłówek.' },
      { re: /piej[ąa]\s+(z\s+)?zachwytu/i, snark: '"{0}" — kilka osób powiedziało, że ok.' },
      { re: /widzowie\s+oniemiel/i, snark: '"{0}" — czyli paru osobom się podobało. Ale to mogło być kilka osób.' },
      { re: /zaskoczyła?\s+wszystkich/i, snark: '"{0}" — wszyscy już wiedzą, a ty nie. Teraz musisz kliknąć, żeby nie zostać w tyle. Sprytne.' },
    ],
  },
  {
    ...
    rules: [
      ...
      { re: /wzrusz/i, exclude: /bez\s+wzruszeń|brak\s+wzruszeń/i, snark: '"{0}" — wzruszy cię bardziej rachunek za prąd.' },
      ...
      { re: /przeraża\s+(mnie|nas|go|ją)/i, snark: '"{0}" — w cytacie bez wyjaśnienia = emocja zamiast informacji.' },
      { re: /łamie\s+serce/i, snark: '"{0}" — komu? O kim mowa? Każą nam się wzruszać, żebyśmy klikali.' },
      { re: /^smutn[eayo]\s+doniesieni/i, snark: '"{0}" — nie chcą podać przyczyn, bo przyczyny są banalne.' },
      { re: /^przerażając/i, snark: '"{0}" — zapowiedź emocji zamiast informacji.' },
      { re: /pustoszy\s+(portfel|organizm|zdrowie)/i, snark: '"{0}" — ale CO? Nie powiedzą, bo wtedy nie klikniesz.' },
    ],
  },
  {
    id: 'challenge',
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /piej[ąa]\s+(z\s+)?zachwytu/i, snark: '"{0}" — kilka osób powiedziało, że ok.' },
      { re: /przytyka/i, snark: '"{0}" — "przytyka" = powiedział coś krytycznego. Ale "powiedział" to za mało na nagłówek.' },
      { re: /niszczy\s+(państwo|polskę|demokrację|gospodarkę|kraj|naród)/i, snark: '"{0}" — "niszczy" = ocenia negatywnie. Ale "ocenia negatywnie" to za mało na nagłówek, więc wyolbrzymiamy.' },
    ],
  },
  {
    id: 'underpromise',
    ...
    rules: [
      ...
      { re: /pokochasz\s+(ten|to|tę|te)\b/i, snark: '"{0}" — nie znamy cię, nie wiemy co lubisz. Jak klikniesz, to sprawdzimy.' },
      { re: /^ekspert\s+(mówi|twierdzi|radzi|ostrzega)/i, snark: '"{0}" — eksperci mówią. To ich zawód. Ale CO mówi — tego tytuł nie zdradzi.' },
      { re: /^brudn[eayo]\s+(praktyki|metody|zagrywki)/i, snark: '"{0}" — jakie? Nie powiedzą, bo wtedy nie klikniesz.' },
      { re: /nie\s+miel[aiy]\s+wstydu/i, snark: '"{0}" — skąd wiadomo? Ktoś ich zapytał?' },
      { re: /zakpił[aoy]?\s+(z|sobie)/i, snark: '"{0}" — ktoś powiedział coś śmiesznego. Ale "powiedział coś śmiesznego" to nie nagłówek.' },
      { re: /chodzi\s+o\s+(spore|duże|ogromne|niemałe)\s+pieniądze/i, snark: '"{0}" — jakie, czyje, ile? Nie powiedzą, bo kwota pewnie nie robi wrażenia.' },
      { re: /^niezwykł[yae]\s+(widok|gest|odkrycie|scena|historia)/i, snark: '"{0}" — mogli opisać jaki. Nie zrobili, bo opis nie klika się tak dobrze.' },
      { re: /nagranie\s+podbija\s+(sieć|internet)/i, snark: '"{0}" — podbija = jest dostępne. Ale "jest dostępne" nie klika się tak dobrze.' },
      { re: /^kolejn[yaeio]\s+/i, snark: '"{0}" — "kolejny" zamiast nazwy — bo nazwa nie klikałaby się tak dobrze.' },
    ],
  },
  {
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /\btyle\b.{0,30}(kosztuje|kosztowało|zapłacił|zapłaciła|otrzymuj[eą]|zarabia|dostaj[eą]|wynosi|dostan[ąa]|strac[ąai]|utrac[ąai]|zapłac[ąai])/i, exclude: /\d{2,}\s*(zł|tys|mln|mld|proc|%|euro|dolar)/i, snark: '"{0}" — tyle, ale ile? Nie podali kwoty, bo wtedy nie klikniesz.' },
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /^znan[yiae]\s+(aktor|dziennik|celebry|polity|trener|piosenk|sportow|model|influenc|bloger|youtuber|lekarz|prawni|biznesm|milioner)/i, snark: '"{0}" — gdyby naprawdę znany, podaliby nazwisko. Anonimowa sława to oksymoron.' },
      { re: /^popularn[yiae]\s+(aktor|dziennik|celebry|polity|trener|piosenk|sportow|model|influenc|bloger|youtuber|lekarz|prawni)/i, snark: '"{0}" — popularna, ale bez nazwiska? Czyli popularność jest dyskusyjna.' },
      { re: /^słynn[yiae]\s+(aktor|dziennik|celebry|polity|trener|piosenk|sportow|model|influenc)/i, snark: '"{0}" — słynna, ale bez nazwiska? Sława wystarczyłaby na podanie imienia.' },
      { re: /^ulubienn?[yiae]\s+(aktor|dziennik|celebry|polity|trener|piosenk|sportow)/i, snark: '"{0}" — czyj ulubiony? Ilu Polaków? Pewnie nie aż tylu.' },
      { re: /^był[aoy]?\s+gwiazd[ąa]/i, snark: '"{0}" — była gwiazdą, ale redakcja boi się, że nie wiesz którą. Gdybyś wiedział, podaliby nazwisko.' },
      { re: /^legendarn[yiae]\s+(aktor|opozycjoni|polity|trener|piosenk|sportow|dziennik|generał|wojskow|duchown|reżyser|naukowiec|profesor|pisarz|muzyk|lekarz)/i, snark: '"{0}" — legendarny, ale bez nazwiska. Gdyby był legendarny, nazwa wystarczyłaby za cały tytuł.' },
      { re: /bez\s+(czołowej|głównej|największej)\s+gwiazdy/i, snark: '"{0}" — bez gwiazdy, ale której? I bez tytułu — bo wtedy nie klikniesz.' },
      { re: /kultow[yiae]\s+(serial|film|program|audycj|gra|zespół)/i, snark: '"{0}" — kultowy, ale bez tytułu. Gdyby był kultowy, nazwa wystarczyłaby za cały nagłówek.' },
    ],
  },
  {
    id: 'caps_exclaim',
  ...
  },
  {
    ...
    rules: [
      ...
      { re: /stawi[ał]?\s+(go|ją|ich|nas)?\s*do\s+pionu/i, snark: '"{0}" — "stawia do pionu" = nie zgadza się. W realu nikt nikogo nie stawia, ale w nagłówku chętnie.' },
    ],
  },
];
